...
CXX_DEFS = -DSTM32F10X_MD -DUSE_STDPERIPH_DRIVER -DHSE_VALUE=8000000 

# C definitions
C_DEFS = -DSTM32F10X_MD -DUSE_STDPERIPH_DRIVER -DHSE_VALUE=8000000 -DSystemInit=low_level_init_1
...
INC_DIRS = $(PROJECT_PATH)Libraries/STM32F10x_StdPeriph_Driver/inc \
$(PROJECT_PATH)inc \
$(PROJECT_PATH)Libraries/CMSIS/CM3/CoreSupport \
$(PROJECT_PATH)Libraries/CMSIS/CM3/DeviceSupport/ST/STM32F10x
...
SRCS_DIRS = $(PROJECT_PATH)Libraries/STM32F10x_StdPeriph_Driver/src \
$(PROJECT_PATH)Libraries/CMSIS/CM3/CoreSupport \
$(PROJECT_PATH)system \
$(PROJECT_PATH)src
...
CXX_FLAGS = -std=$(CXX_STD) $(MODE) -fno-rtti -fno-exceptions -fverbose-asm -Wa,-ahlms=$(OUT_DIR_F)$(notdir $(<:.$(CXX_EXT)=.lst))

# flags for C compiler
C_FLAGS = -std=$(C_STD) $(MODE) -fverbose-asm -Wa,-ahlms=$(OUT_DIR_F)$(notdir $(<:.$(C_EXT)=.lst))

# flags for assembler
AS_FLAGS = $(MODE) -Wa,-amhls=$(OUT_DIR_F)$(notdir $(<:.$(AS_EXT)=.lst))

# flags for linker
LD_FLAGS = -T$(LD_SCRIPT) $(MODE) -Wl,-Map=$(OUT_DIR_F)$(PROJECT).map,--cref,--no-warn-mismatch
